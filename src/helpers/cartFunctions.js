...
export const loadCartFromLocalStorage = () => {
  const productsInLocalStorage = getSavedCartIDs();
  const allProductsPromises = productsInLocalStorage
    .map((itemId) => fetchProduct(itemId));

  const productsList = Promise.all(allProductsPromises);
